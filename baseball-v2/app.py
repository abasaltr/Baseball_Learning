...
class PlayerPredict(db.Model):
    __tablename__ = 'PlayerPredictions'

    ID = db.Column(db.Integer, primary_key=True)
    fpID = db.Column(db.Integer)
    yearID = db.Column(db.Integer)
    actual_h = db.Column(db.Float)
    model_h = db.Column(db.Float)
    actual_r = db.Column(db.Float)
    model_r = db.Column(db.Float)
    actual_hr = db.Column(db.Float)
    model_hr = db.Column(db.Float)
    actual_bb = db.Column(db.Float)
    model_bb = db.Column(db.Float)
    modelType = db.Column(db.String(11))
...
def playerPredict(fp_id):
    ...
    results = db.session.query(PlayerPredict.ID, PlayerPredict.fpID, PlayerPredict.yearID, 
                        ...
                        PlayerPredict.actual_r, 
                        PlayerPredict.model_r,
                        PlayerPredict.actual_hr, 
    ...
    y_id = [result[2] for result in results]
    actual_h = [result[3] for result in results]
    model_h = [result[4] for result in results]
    actual_r = [result[5] for result in results]
    model_r = [result[6] for result in results]
    actual_hr = [result[7] for result in results]
    model_hr = [result[8] for result in results]
    actual_bb = [result[9] for result in results]
    model_bb = [result[10] for result in results]
    modelType = [result[11] for result in results]
